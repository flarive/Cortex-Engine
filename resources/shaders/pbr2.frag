...
float ShadowCalculationCubeMap(vec3 fragPos, vec3 lightPos)
{
    ...
    // Correct cubemap lookup: use unnormalized direction
    float closestDepth = texture(texture_shadowMapCube, fragToLight).r * far_plane;

    // Bias based on geometric normal (same as Blinn-Phong)
    float bias = max(material.shadowMapsBias * 
                     (1.0 - dot(normalize(Normal), normalize(fragToLight))),
                     material.shadowMapsBias * 0.25);

    // Distance-based PCF radius
    ...
    {
        vec3 sampleDir = fragToLight + sampleOffsets[i] * diskRadius;
        float depth = texture(texture_shadowMapCube, sampleDir).r * far_plane;
...
//
//    // ------------------------------------------------------------
//    // 2. Spotlight cone falloff
//    // ------------------------------------------------------------
//    float theta = dot(L, normalize(-light.direction));   // verify CPU-side!
//    float spotFactor = smoothstep(light.outerCutOff, light.cutOff, theta);
//
//    if (spotFactor <= 0.0)
//        return vec3(0.0);
//
//    float spotIntensity = spotFactor * light.intensity;
//
//    // ------------------------------------------------------------
//    // 3. Attenuation
//    // ------------------------------------------------------------
...
//    float shadow = 0.0;
//
//    if (enableShadows && material.canReceiveShadows && light.use)
//    {
//        if (material.shadowCalculationMethod == 1)
//            shadow = ShadowCalculationPCF(FragPosLightSpace, geoNormal, light.direction);
//        else if (material.shadowCalculationMethod == 2)
//            shadow = ShadowCalculationSoft(FragPosLightSpace, geoNormal, light.direction);
//        else if (material.shadowCalculationMethod == 3)
//            shadow = ShadowCalculationPCSS(FragPosLightSpace, geoNormal);
//    }
//
//    shadow = clamp(shadow * material.shadowIntensity, 0.0, 1.0);
//    radiance *= (1.0 - shadow);
...
{
    // ------------------------------------------------------------
    // 1. Light direction, distance, attenuation (Blinn‑Phong exact)
    // ------------------------------------------------------------
    vec3 L = normalize(light.position - fragPos);
    float distance = length(light.position - fragPos);
    ...
    // ------------------------------------------------------------
    // 2. Shadow (your corrected cubemap version)
    // ------------------------------------------------------------
    float shadow = 0.0;
    if (enableShadows && material.canReceiveShadows && light.use)
        shadow = ShadowCalculationCubeMap(fragPos, light.position);

    shadow = clamp(shadow * material.shadowIntensity, 0.0, 1.0);

    // ------------------------------------------------------------
    // 3. Radiance (Blinn‑Phong style, but PBR‑correct)
    // ------------------------------------------------------------
    ...
    vec3 numerator   = NDF * G * F;
    float denom      = 4.0 * max(NdotV, 0.0) * max(NdotL, 0.0) + 0.0001;
    vec3 specular    = numerator / denom;
...
vec3 CalcDirLight(DirLight light, vec3 normal, vec3 geoNormal, vec3 fragPos, vec2 texCoords, vec3 viewDir, vec3 albedo)
{
    // ------------------------------------------------------------
    // 1. Light direction (directional lights have no attenuation)
    // ------------------------------------------------------------
    ...
    // ------------------------------------------------------------
    // 3. Radiance (directional lights have no attenuation)
    // ------------------------------------------------------------
    ...
    // Base reflectance
    vec3 F0 = mix(vec3(0.04), albedo, 0.0); // directional lights do NOT use metallic workflow
