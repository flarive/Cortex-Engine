...
void engine::ImGuiEditor::setScene(std::shared_ptr<Entity> rootEntity)
{
    ...
    auto* dockspace = m_ui.create<DockSpaceElement>();
    
    auto* sceneWindow = m_ui.create<SceneWindow>();
    sceneWindow->setRootEntity(m_rootEntity);

    auto* settingsWindow = m_ui.create<SettingsWindow>();
    
    auto* aboutWindow = m_ui.create<AboutWindow>();
    aboutWindow->init();
