    ...
    if (m_sphereVAO == 0)
    {
        ...
        for (unsigned int x = 0; x <= X_SEGMENTS; ++x)
        {
            for (unsigned int y = 0; y <= Y_SEGMENTS; ++y)
            {
                ...
                float xPos = std::cos(xSegment * 2.0f * constants::PI) * std::sin(ySegment * constants::PI);
                float yPos = std::cos(ySegment * constants::PI);
                float zPos = std::sin(xSegment * 2.0f * constants::PI) * std::sin(ySegment * constants::PI);
