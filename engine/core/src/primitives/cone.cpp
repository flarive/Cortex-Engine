...
namespace engine
{
    void Cone::setup(const std::shared_ptr<Material>& material)
    {
        m_material = material;
    ...
    void Cone::setup(const std::shared_ptr<Material>& material, const UvMapping& uv)
    {
        m_material = material;
    ...
    std::vector<Vertex> engine::Cone::generateVertices()
    {
        return generateConeVertices(36, height, radius, m_uvScale);
    }

    void Cone::draw(Shader& shader, const glm::vec3& position, const glm::vec3& size, const glm::vec3& rotation)
    {
        shader.use();
        if (m_material)
        {
    ...
    void Cone::draw(Shader& shader, const glm::mat4 model)
    {
    }
}
