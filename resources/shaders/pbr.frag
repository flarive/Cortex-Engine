...
float ShadowCalculationSoft(vec4 fragPosLightSpace, vec3 geoNormal, vec3 lightPos)
{
    ...
    float bias = max(0.0005 * (1.0 - dot(normalize(geoNormal), normalize(lightPos - FragPos))), 0.0001);
...
//    vec3 mSpecular = vec3(0.23f, 0.23f, 0.23f);
//
//    vec4 diffuseSample = material.has_texture_diffuse_map
...
//    {
//        // 2 distinct textures
//        metallic = material.has_texture_metalness_map ? texture(material.texture_metalness, texCoords).r : 1.0;
//        roughness = material.has_texture_roughness_map ? texture(material.texture_roughness, texCoords).r : 0.5;
//    }
//
